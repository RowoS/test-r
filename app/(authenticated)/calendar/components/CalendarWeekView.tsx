...
export function CalendarWeekView({ weekOf, events, onSlotClick, onEventClick }: Props) {
  ...
  useEffect(() => {
    scrollRef.current?.scrollTo({ top: 7 * HOUR_HEIGHT }); // open on business hours, not midnight
  }, []);
              ...
              >
                ...
                {positioned.map(({ event, top, height, lane, laneCount }) => (
                  <div
                    ...
                    className={`absolute overflow-hidden rounded border px-1 py-0.5 text-[11px] leading-tight ${EVENT_COLORS[event.event_type] ?? EVENT_COLORS.other}`}
